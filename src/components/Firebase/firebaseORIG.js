...
class Firebase {
  ...
  populateCard = exercise => {
    // console.log(`this.auth.currentUser.uid is ${this.auth.currentUser.uid}\nexerciseId is ${exercise.id}\nworkout is ${exercise.workout}`);
    // const exerciseRef = this.db.ref(`${exercise.workout}/${exercise.id}`);

    // TODO: Need to get exercise id generated by push() to include in ref address
    const exerciseRef = this.db.ref(`${exercise.workout}/`);
    return exerciseRef;
  }
  ...
  saveExercise = (exerciseId, workout, exercise, reps, weight) => {
    ...
    // this.db.ref(`${workout}/${exerciseId}`)
    //   .set({
    //     exercise,
    //     userId,
    //     reps,
